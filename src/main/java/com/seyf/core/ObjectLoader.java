...
public class ObjectLoader {
    ...
    public Model loadOBJModel(String filename) {
        ...
        float[] textCoordsArr = new float[vertices.size()*2];
        float[] normalArr = new float[vertices.size()*3];

        for (Vector3i face:faces){
             processVertex(face.x,face.y,face.z,textures,normals,indices,textCoordsArr,normalArr);
        }

        int[] indicesArr= indices.stream().mapToInt((Integer v)->v).toArray();

        return loadModel(verticesArr,textCoordsArr, normalArr ,indicesArr);

    }



    public static void processVertex(int pos, int textCorrd, int normal,List<Vector2f> textCoorsList,
                                     List<Vector3f> normalList, List<Integer> indecesList,
                                     float[] textCoordArr, float[] normalArr){

        indecesList.add(pos);

        if(textCorrd>=0){
            Vector2f textCoordVec = textCoorsList.get(textCorrd);
